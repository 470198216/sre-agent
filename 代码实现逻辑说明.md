...
1. 若工具需要 `path`：必须提供，且 `path in allowed_paths`  
2. 用 `tool.command.format(path=path)` 填模板（模板是人写死的）  
3. 再扫 `FORBIDDEN_SUBSTRINGS`（重定向、`;`、`&&`、危险命令等）  
4. 通过后才返回最终 command 字符串

因此攻击面是：

- 模型只能影响：**选哪个工具名**、**选哪个允许的 path**  
- 不能：拼接 `rm -rf`、加 `>` 写文件（禁词 + 模板双重限制）
